...
What it does:
		Runs the LUA Garbage Collector if the garbage memory count excedes the
	specified limit.
	...
	rig, but much of that is being taken up by persistant things that don't get
	garbage collected. When something happens that makes LUAJIT excede it's
	memory limit it crashes.
		From what I can decern, when you see a LUA_OOM crash the value that is
	reported to you is not the ammount of memory LUAJIT is using, it is the
	ammount of memory that can potentially be freed with a garbage collection
	...
	something happened to cause LUAJIT to excede it's memory limit and... BOOM!
...
Configuration:
	...
	count excedes that number the Garbage Collector will be run. Change the
	...
	run, resuting in slower performance and possibly unfinished collections or
